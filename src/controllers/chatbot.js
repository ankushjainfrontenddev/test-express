...
async function insertDataIntoMongoDB(req, gptResp) {
    try {
        ...
        const dataToInsert = { query: query, response: {}, userId: userId, response: responseFromGPT };
        await Promise.all([
            db.models.ChatbotQuery.create(dataToInsert),
...
async function getOpenAIResponse(query, userId) {
    try {
        ...
        const userListResponse = await db.models.ChatbotQuery.findAll({
            where: { userId: userId },
        });
...
export const getResponse = async (req, res, next) => {
    try {
        ...
        insertDataIntoMongoDB(req, chatCompletion)
            .then(() => {
...
export const cretaeUser = async (req, res, next) => {
    try {
        const { id, ip, userAgent, name, email, company } = req.body;
        // Try to find a user with the provided 'id'
        let user = await db.models.CacheUser.findOne({ where: { id } });
        if (user) {
            ...
            user = await db.models.CacheUser.create({
                id,
                ip,
                userAgent,
                name,
